        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <span>{{ __('Chỉnh sửa khoa') }}</span>
                    <a href="{{ route('faculties.index') }}" class="btn btn-secondary btn-sm">
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('faculties.update', $faculty->id) }}">
                        ...
                        <div class="mb-3">
                            <label for="name" class="form-label">{{ __('Tên khoa') }} <span class="text-danger">*</span></label>
                        ...
                        <div class="mb-3">
                            <label for="code" class="form-label">{{ __('Mã khoa') }} <span class="text-danger">*</span></label>
                            <input type="text" class="form-control @error('code') is-invalid @enderror" id="code" name="code" value="{{ old('code', $faculty->code) }}" required>
                            @error('code')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text">{{ __('Mã khoa phải là duy nhất và không có khoảng trắng.') }}</div>
                        </div>
